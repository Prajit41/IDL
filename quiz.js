...
function renderQuizzes(filterAge) {
  ...
  filtered.forEach((quiz) => {
    ...
    card.querySelector(".learn-btn").addEventListener("click", () => {
      const topicEncoded = encodeURIComponent(quiz.title);
      const ageEncoded = encodeURIComponent(quiz.age);
      window.location.href = `learn.html?topic=${topicEncoded}&age=${ageEncoded}`;
    });
